<script lang="ts">
  ...
  import { getInstructivoParaSeccion } from '$lib/sarlaft/instructivo'
  ...
  let fase = $state<Fase>(data.formulario ? 'intro' : 'intro')
  ...
  function esPreguntaVisible(preguntaId: string): boolean {
    ...
    if (seccion.tipo_bloque === 'tabla_repetible_multiple') {
      ...
      for (let i = 0; i < filas.length; i++) {
        for (const p of seccion.preguntas) {
          if (p.obligatorio && (!filas[i][p.id] || filas[i][p.id] === '')) {
      ...
      for (const p of seccion.preguntas) {
        ...
        if (!esPreguntaVisible(p.id)) continue
  ...
  async function handleSubmit() {
    ...
    try {
      const result = await sarlaftApi.submit({
        ...
        fecha_diligenciamiento: respuestas[formulario.secciones[0].preguntas[0].id] || undefined,
        respuestas: payload,
  ...
  const instructivoSecciones = $derived(seccionActual ? getInstructivoParaSeccion(seccionActual.seccion) : [])
...
<svelte:head>
  <title>
    {formulario ? `${formulario.titulo} | TRANSMERALDA S.A.S.` : 'Formularios SARLAFT + PTEE | TRANSMERALDA S.A.S.'}
  </title>
  <meta
    name="description"
            ...
            <div class="full">
              ...
              <div class="section-fields">
                {#each seccionActual.preguntas as pregunta (pregunta.id)}
                  {#if esPreguntaVisible(pregunta.id)}
